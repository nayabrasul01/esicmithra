...
const ReferralTable = ({ user, refreshTrigger, onEdit }) => {
  ...
  useEffect(() => {
    const getReferralRequest = async () => {
      try {
        const res = await fetchReferrals(user?.location?.id);
    ...
    getReferralRequest();
  }, [user?.location?.id, refreshTrigger]);

  const filterReferrals = (search) => {
    const result = referrals.filter(
      (r) =>
  ...
  const getPatientLiveListData = async (ipNumber) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const list = selfMember.uHID
          ? [selfMember, ...familyMembers]
          : familyMembers;
